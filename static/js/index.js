...
function displayNews(news) {
    ...
    container.innerHTML = sortedNews.map(item => `
        <div class="bg-white rounded-lg shadow p-6 mb-4 cursor-pointer hover:shadow-md transition-shadow" onclick="selectNews(${item.id})">
    ...
    try {
        ...
        for (let i = 0; i < summarizedData.summaries.length; i++) {
            ...
            const newsItem = {
                ...
                content: originalText, // 전체 내용 저장
                published_date: new Date(newsPubDate).toISOString(), // ISO 형식으로 저장
                sentiment_category: sentimentCategory, // 필터링을 위한 카테고리
                sentiment_scores: {
                    positive: sentimentCategory === 'positive' ? 0.8 : 0.1,
                    negative: sentimentCategory === 'negative' ? 0.8 : 0.1,
                    neutral: sentimentCategory === 'neutral' ? 0.8 : 0.1
                }
            };
            allNews.push(newsItem);
            
            // 세션 스토리지에 뉴스 데이터 저장 (최대 50개 유지)
            const storedNews = JSON.parse(sessionStorage.getItem('newsData') || '{}');
            storedNews[newsItem.id] = newsItem;
            // 오래된 데이터 정리 (가장 오래된 항목 제거)
            const storedIds = Object.keys(storedNews).map(Number);
            if (storedIds.length > 50) {
                const oldestId = Math.min(...storedIds);
                delete storedNews[oldestId];
            }
            sessionStorage.setItem('newsData', JSON.stringify(storedNews));
        }
...
/**
 * 뉴스 상세 페이지로 이동하는 함수
 * @param {number} newsId - 상세 정보를 볼 뉴스의 ID
 */
function selectNews(newsId) {
    window.location.href = `/news/${newsId}?id=${newsId}`;
}
