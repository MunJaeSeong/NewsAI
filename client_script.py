...
BASE_API_URL = "http://127.0.0.1:8000/api" # API 경로에 /api 추가

# 감성별 개수를 저장할 딕셔너리 초기화
sentiment_counts = {
    "긍정": 0,
    "부정": 0,
    "중립": 0,
...
try:
    # 1단계: 통합 검색 API 호출
    search_url = f"{BASE_API_URL}/search/" # /api/search/
    ...
    if search_data.get("status") == "success":
        ...
        if descriptions:
            ...
            # 2단계: 모든 description을 통합 요약 API에 한 번에 전송
            summarize_url = f"{BASE_API_URL}/summarize/multiple" # /api/summarize/multiple
            ...
            if summarized_data.get("status") == "success":
                for i, item in enumerate(summarized_data.get("summaries", [])):
                    ...
                    if summary_text is not None:
                        ...
                        # 3단계: 요약된 텍스트를 통합 감성 분석 API에 전송
                        sentiment_url = f"{BASE_API_URL}/sentiment/analyze" # /api/sentiment/analyze
                        ...
                        try:
                            ...
                            print(f"감성 분석 결과: {final_sentiment} (원문: {sentiment_data.get('original_sentiment', 'N/A')}, 신뢰도: {sentiment_data.get('confidence_score', 'N/A')})")
